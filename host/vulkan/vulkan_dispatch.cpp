...
void VulkanDispatchImpl::initialize(bool forTesting) {
    ...
    // The Android Vulkan loader does not expose the global commands (vkCreateInstance,
    // vkEnumerateInstance*) as dlsym-able symbols; per the loader spec they are queried through
    // vkGetInstanceProcAddr(nullptr, name). The generated dispatch init uses plain dlsym and is
    // autogenerated, so the fix-up belongs here. On a desktop loader these are exported directly
    // and none of this fires.
    if (mDispatch.vkGetInstanceProcAddr) {
        if (!mDispatch.vkCreateInstance) {
            mDispatch.vkCreateInstance =
                (PFN_vkCreateInstance)mDispatch.vkGetInstanceProcAddr(nullptr, "vkCreateInstance");
        }
        if (!mDispatch.vkEnumerateInstanceExtensionProperties) {
            mDispatch.vkEnumerateInstanceExtensionProperties =
