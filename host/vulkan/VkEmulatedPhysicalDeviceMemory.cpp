...
EmulatedPhysicalDeviceMemoryProperties::EmulatedPhysicalDeviceMemoryProperties(
    ...
    //
    // On a UMA GPU (e.g. Adreno) every memory type is HOST_VISIBLE, and the guest driver
    // reacts to HOST_VISIBLE by eagerly creating a coherent blob mapped through the
    // virtio-gpu PCI BAR. That makes every guest VkDeviceMemory allocation consume BAR
    // space, capping total GPU memory at --gpu pci-bar-size (e.g. 256MB) even though the
    // heap is much larger. Large allocations (model weights, big textures) then fail with
    // "Failed to allocate coherent memory".
    //
    // A shadow type advertises only DEVICE_LOCAL, so the guest takes the pass-through path
    // in ResourceTracker::on_vkAllocateMemory (`!requestedMemoryIsHostVisible`) and never
    // maps it: the allocation lives purely in host GPU memory, costs no BAR space, and is
    // filled the normal Vulkan way (staging buffer + vkCmdCopyBuffer). Guests that want a
    // mappable allocation keep using the original host-visible types.
    ...
    if (getenv("GFXSTREAM_DEVICE_LOCAL_MEMORY_TYPE") != nullptr) {
        ...
        for (uint32_t hostMemoryTypeIndex = 0;
            ...
            // Allocations of the shadow use the real host type; only the guest-visible
            // flags differ. The reverse (host -> guest) map is left pointing at the
            // original host-visible type so existing lookups are unchanged.
...
EmulatedPhysicalDeviceMemoryProperties::getHostMemoryInfoFromGuestMemoryTypeIndex(
    ...
    // Allocate from the real host memory type, but report the shadow's device-local-only
    // flags: callers use these flags to decide whether to set up host-visible emulation
    // (shared memory + guest mapping), which a shadow allocation must not do.
...
void EmulatedPhysicalDeviceMemoryProperties::transformToGuestMemoryRequirements(
    ...
    for (uint32_t hostMemoryTypeIndex = 0;
        ...
        // A resource that can live in a host-visible host type can equally live in that
        // type's device-local-only shadow, so offer both to the guest.
...
bool EmulatedPhysicalDeviceMemoryProperties::isDeviceLocalOnlyShadow(
    uint32_t guestMemoryTypeIndex) const {
    if (guestMemoryTypeIndex >= VK_MAX_MEMORY_TYPES) {
        return false;
    }
    return mGuestMemoryTypeIsShadow[guestMemoryTypeIndex];
}
