...
// Promoted-to-core entry points and the KHR/EXT names they came from. A driver may expose either
// spelling (vkGetInstanceProcAddr returns null for a core entry above the negotiated API version,
// and for a KHR entry whose extension is not enabled), so whichever exists fills in for the other.
#define GFXSTREAM_VK_DISPATCH_ALIASES(X) \
    ...
    X(vkGetPhysicalDeviceExternalSemaphoreProperties, vkGetPhysicalDeviceExternalSemaphorePropertiesKHR) \
